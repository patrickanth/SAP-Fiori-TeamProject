...
class DataAccess {
    ...
    async performAction(odataRequest, actionData) {
        ...
        if (rootEntitySet) {
            ...
            const actionDefinition = this.metadata.getVersion() === '2.0'
    ...
    getNavigationPropertyKeys(data, navPropDetail, currentEntityType, currentEntitySet, currentKeys, forCreate = false) {
        if (navPropDetail.referentialConstraint && navPropDetail.referentialConstraint.length > 0) {
            ...
            const backNav = navPropDetail.targetType.navigationProperties.find((targetNavProp) => targetNavProp.targetTypeName === currentEntityType.fullyQualifiedName);
        ...
        if (data !== null || (Array.isArray(data) && data.length > 0)) {
            ...
            if (this.metadata.getVersion() === '2.0') {
                const enrichElement = (entitySet, entityType, dataLine, originalDataLine) => {
                    ...
                    const uri = `${this.service.urlPath}/${entitySet ? entitySet.name : entityType.name}(${keyValues.join(',')})`;
                    dataLine['__metadata'] = {
                        id: uri,
                        uri: uri,
        ...
        if (navPropertyName && entitySet) {
            ...
            if (!navPropDetail.containsTarget) {
                ...
                odataRequest.addResponseHeader('Location', `${targetEntitySet.name}(${Object.keys(currentKeys)
                    ...
                    .join(',')})`);
                postData = (await this.getMockEntitySet(targetEntitySet.name)).performPOST(currentKeys, postData, odataRequest.tenantId, odataRequest, true);
            }
            else {
            ...
            postData = (await this.getMockEntitySet(entitySet.name)).performPOST(currentKeys, postData, odataRequest.tenantId, odataRequest, true);
